...
#%%
from GLMCC.src import util
from GLMCC.src import glmcc
from GLMCC import params
...
def glm_plot(data_dir, prefix, extension, target_id, reference_id, save_dir, start, end, window, synaptic_delay):
    if target_id == reference_id:
        ...
        bool = np.abs(optimal_glm.theta[-2:]) >= optimal_glm.j_thresholds
        if bool[0] != False and bool[1] != False:
            return True
